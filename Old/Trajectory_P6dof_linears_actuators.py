...
B1 = np.array(
    [-R * np.cos(np.radians(0 - omega)), R * np.sin(np.radians(0 - omega)), 0]
)
B2 = np.array(
    [-R * np.cos(np.radians(0 + omega)), R * np.sin(np.radians(0 + omega)), 0]
)
B3 = np.array(
    [-R * np.cos(np.radians(120 - omega)), R * np.sin(np.radians(120 - omega)), 0]
)
B4 = np.array(
    [-R * np.cos(np.radians(120 + omega)), R * np.sin(np.radians(120 + omega)), 0]
)
B5 = np.array(
    [-R * np.cos(np.radians(240 - omega)), R * np.sin(np.radians(240 - omega)), 0]
)
B6 = np.array(
    [-R * np.cos(np.radians(240 + omega)), R * np.sin(np.radians(240 + omega)), 0]
)
P1 = np.array(
    [-r * np.cos(np.radians(0 - sigma)), r * np.sin(np.radians(0 - sigma)), 0]
)
P2 = np.array(
    [-r * np.cos(np.radians(0 + sigma)), r * np.sin(np.radians(0 + sigma)), 0]
)
P3 = np.array(
    [-r * np.cos(np.radians(120 - sigma)), r * np.sin(np.radians(120 - sigma)), 0]
)
P4 = np.array(
    [-r * np.cos(np.radians(120 + sigma)), r * np.sin(np.radians(120 + sigma)), 0]
)
P5 = np.array(
    [-r * np.cos(np.radians(240 - sigma)), r * np.sin(np.radians(240 - sigma)), 0]
)
P6 = np.array(
    [-r * np.cos(np.radians(240 + sigma)), r * np.sin(np.radians(240 + sigma)), 0]
)
...
def plot_circle(ax, center, radius, normal, color="k", linestyle="--"):
    ...
    v = (
        ...
        else np.array([0, 1, 0], dtype=float)
    )
...
def linear_actuator_trajectory(roll_amp, pitch_amp, yaw_amp, xf, yf, zf, steps):
    ...
    def update(frame):
        ...
        T = np.array(
            [x, y, z]
        )  # Vector corresponding to the distance between the center of the platform and the base
        ...
        ax.scatter(
            P[:, 0], P[:, 1], P[:, 2], color="#72bdba", s=100, label="Cylinder Bodies"
        )
        ax.scatter(
            P[:, 0], P[:, 1], P[:, 2], color="#ffd783", s=100, label="Cylinder Strokes"
        )
        ax.scatter(
            P[:, 0], P[:, 1], P[:, 2], color="#f2887c", s=100, label="3DOF Platform"
        )
        ...
        for i in range(6):
            ...
            ax.text(
                B[i, 0] + lab / L[i] * (P[i, 0] - B[i, 0]) + L_offset,
                B[i, 1] + lab / L[i] * (P[i, 1] - B[i, 1]) + L_offset,
                B[i, 2] + lab / L[i] * (P[i, 2] - B[i, 2]) + L_offset,
                f"L{i+1}",
                color="#5f7fbf",
                fontsize=12,
                fontweight="bold",
        ...
        plot_circle(
            ax, T, r, Rot_BtoP @ np.array([0, 0, 1]), color="#f2887c", linestyle="--"
        )
